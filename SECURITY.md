...
`convert-color` runs entirely in your browser. There is no server component and no
account system, so the color values you type are never uploaded. Most classic web
...
dependencies, a service worker caching bug, or anything that could cause your input
...
- The HEX/RGB/HSL conversion engine, the service worker, and the PWA manifest.
- Anything that could send the color values you type off the device.
