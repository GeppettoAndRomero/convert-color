...
export async function registerServiceWorker(): Promise<ServiceWorkerRegistration | null> {
  ...
  try {
    // Service Workerを登録（/convert-color/sw.js, scope=/convert-color/）。
    // scope がツールの全 locale ページ（/convert-color/, /convert-color/ja/ …）を被覆する。
    const registration = await navigator.serviceWorker.register('/convert-color/sw.js', {
      scope: '/convert-color/',
    });
